"""Regression: a single permission-denied directory must not abort
expected_source_paths() (the health / verify coverage check).

`expected_source_paths()` collected Python files with `d.rglob("*.py")` for
...
def test_unreadable_dir_does_not_abort_expected_paths(two_source_dirs):
    # Pre-fix this raises PermissionError out of expected_source_paths().
    paths = embeddings.expected_source_paths()
    assert "good/ok.py" in paths, "readable source must still be reported"
