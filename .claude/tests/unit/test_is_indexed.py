...
class TestParity:
    ...
    def test_md_under_source_dir_not_gated(self, search_first, index_refresh):
        ...
        assert sf_indexed(search_first, "tools/notes.md") == ir_indexed(
            index_refresh, "tools/notes.md"
        )
        assert sf_indexed(search_first, "tools/notes.md") is False

    def test_non_indexed_extension(self, search_first, index_refresh):
        assert sf_indexed(search_first, "tools/data.json") == ir_indexed(
            index_refresh, "tools/data.json"
        )
    ...
    def test_root_level_exclusion(self, search_first, index_refresh):
        ...
        assert sf_indexed(search_first, "legacy/foo.py") == ir_indexed(
            index_refresh, "legacy/foo.py"
        )
