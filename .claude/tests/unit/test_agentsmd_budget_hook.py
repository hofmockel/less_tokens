...
sys.path.insert(
    0, str(HOOK.parent)
)  # agentsmd-budget.py does `from _codex_runtime import ...`
    ...
    def test_agentsmd_budget_always_present(self, tmp_path):
        ...
        for caveman in (True, False):
            entries = build_codex_hook_entries(
                venv_py, tmp_path, _codex_args(caveman=caveman)
            )
...
def _run_main(
    ...
    mod.cma = cma
    with patch.object(mod.sys, "stdin", MagicMock(read=lambda: json.dumps(stdin_data))):
        import io
...
class TestAgentsMdBudgetHook:
    ...
    def test_returns_2_when_over_budget(self, tmp_path):
        ...
        mod.sys.stdin = io.StringIO(
            json.dumps(
                {"tool_name": "Edit", "tool_input": {"file_path": str(agents_md)}}
            )
        )
        assert mod.main() == 2
    ...
    def test_returns_0_when_under_budget(self, tmp_path):
        ...
        mod.sys.stdin = io.StringIO(
            json.dumps(
                {"tool_name": "Write", "tool_input": {"file_path": str(agents_md)}}
            )
        )
        assert mod.main() == 0
