...
class TestCodexContinueFreshness:
    def test_blocks_stale_handoff_read(self, tmp_path):
        ...
        code, stdout, stderr = run_hook_with_env(
            "continue-freshness.py",
            {
...
class TestCodexTruncateOutput:
    ...
    def test_truncates_large_filesystem_read(self):
        code, stdout, _ = run_hook_with_env(
            "truncate-output.py",
            {
                "tool_name": "mcp__filesystem__read_file",
                "tool_response": "x" * 10_000,
...
class TestCodexTerseReminder:
    ...
    def test_blocks_filler_response(self):
        code, _, stderr = run_hook_with_env(
            "terse-reminder.py",
            {
                "hook_event_name": "Stop",
...
class TestCodexSubagentMetrics:
    def test_records_separate_correlated_sizes(self, tmp_path):
        ...
        records = [
            json.loads(line)
            for line in (tmp_path / "subagent-metrics.jsonl").read_text().splitlines()
        ]
...
class TestCodexPostEditAndReadAfterEdit:
    ...
    def test_read_after_edit_blocks_recent_reread(self, tmp_path):
        ...
        (state_dir / "last-edit.json").write_text(
            json.dumps({str(target.resolve()): __import__("time").time()})
        )
        code, _, stderr = run_hook_with_env(
            "read-after-edit.py",
            {
...
class TestCodexBashAndCacheAdapters:
    ...
    def test_context_cache_blocks_repeat_read(self, tmp_path):
        ...
        code1, _, _ = run_hook_with_env(
            "context-cache.py", pre, extra_env={"LESS_TOKENS_STATE_DIR": str(state_dir)}
        )
        code_post, _, _ = run_hook_with_env(
            "context-cache.py",
            post,
            extra_env={"LESS_TOKENS_STATE_DIR": str(state_dir)},
        )
        code2, stdout2, stderr2 = run_hook_with_env(
            "context-cache.py", pre, extra_env={"LESS_TOKENS_STATE_DIR": str(state_dir)}
        )
        assert code1 == 0
        assert code_post == 0
