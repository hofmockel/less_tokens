...
2. **Read the cited location** — open the file at the referenced line. Confirm the inconsistency
   ...
   step 7 with a "confirmed fixed" note instead of guessing at a new fix.
...
6. **Write the changelog entry first** — add a bullet to the target repo's own changelog
   ...
   not proceed.
7. **Delete the backlog row** — remove the bug's row from the target repo's backlog. No
   strike-through, no "DONE" marker. The row must be gone. Only after the changelog entry is
   confirmed written.
8. **Commit atomically** — stage only the files you edited (no bulk `git add .`/`git add -A`).
   One bug = one commit; never batch two bugs into one commit.
9. **Report** — print the commit hash (or diff, if not committing) and a one-line summary. Loop
   to the next bug only if the user asked for more than one.
...
5. **Write the `CHANGELOG.md` entry first** — one bullet under `## [Unreleased]`: what was wrong,
   what changed, file:line, and the regression test's name. Do this BEFORE touching `BACKLOG.md`.
6. **Delete the backlog row** — remove the bug's row from `BACKLOG.md`. No strike-through, no
   "DONE" marker. Only after the changelog entry is confirmed written.
7. **Commit atomically** — stage only the files you edited. One bug = one commit.
8. **Report** — print the commit hash, the regression test's nodeid, and a one-line summary.
