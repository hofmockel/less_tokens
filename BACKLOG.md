...
| **Bug** | **Details** | **Status** |
|---|---|---|
| Codex hook commands break from a nested cwd (universal, all 16 hooks) | `.codex/hooks.json` commands use a relative path (e.g. `LESS_TOKENS_AGENT=codex .less_tokens/bin/python .codex/hooks/savings-html.py`, generated by `install.py`'s `launcher_cmd`/`_rel_or_abs`). Confirmed by installing into a scratch repo and running the installed command with `cwd` set to a subdirectory: `sh: .less_tokens/bin/python: No such file or directory` (exit 127, no stderr surfaced to the agent). Works fine from repo root. Claude doesn't have this problem — the harness resets Bash's cwd to project root between tool calls; Codex has no such guarantee. Found while building G15's Codex-side acceptance test (2026-07-07). **Confirmed universal 2026-07-08**: all 16 wired `.codex/hooks.json` commands share the identical relative-path launcher pattern, so a nested-cwd tool call silently disables *all* less_tokens enforcement for Codex for that call, not just savings-html — the single largest enforceability risk in the Codex integration. Raise from "whenever convenient" to top Codex-specific priority; fix at the shared launcher-generation layer (pairs with "Codex hook wrappers: collapse repeated adapter boilerplate" below), not per-hook patches — left for a Codex-context session per hofmockel. | Open |
| `savings.html` shows stale/zero data, filed as affecting both agents | Root-caused 2026-07-08: Codex's `.less_tokens/state/savings.html` was genuinely stale — a real event landed in `savings.jsonl` on 2026-07-05 that never triggered regeneration; manually invoking `savings-html.py` regenerated it correctly on the first try, so the script itself isn't broken. High-confidence this is the nested-cwd bug's visible symptom (above), not an independent defect. Claude's `.claude/state/savings.html` was checked too and is **not** actually broken — it held real data as of its last generation, just hadn't regenerated since the last `Stop` event in this repo. No separate fix needed; closes once the nested-cwd bug is fixed. Narrow this row to Codex-only once confirmed. | Likely duplicate of the nested-cwd bug above — pending hofmockel confirming the original observation was made in a Codex context |
...
- **Research why `savings.html` appears to show nothing** *(measurement / UX)* — Do not assume this is only the existing Codex nested-cwd bug until the visible symptom is reproduced. The HTML page is regenerated by `.claude/hooks/savings-html.py` / `agents/codex/hooks/savings-html.py`, which both swallow all errors and only print a link when current-session measured chars are >0; the HTML renderer itself always writes session and all-time panels from `_load_records()` (`.claude/tools/stats.py:522-549`), while the transcript/status headline is measured-only (`.claude/tools/stats.py:572-579`) and intentionally ignores upper-bound events. Research should classify the failure mode: missing/stale `savings.jsonl`, stale `savings.html` mtime after new events, session-window filtering hiding all-time data, measured-vs-upper-bound confusion, browser/cache/opening the wrong agent path (`.claude/state/savings.html` vs `.less_tokens/state/savings.html`), or silent hook import/write failure. Acceptance: add a reproducible diagnostic command or `stats.py --doctor-html` that prints log path, log event count, newest log ts, HTML path/mtime, session vs all-time counts, measured vs upper-bound totals, active agent/state dir, and whether hook regeneration succeeds from repo root and nested cwd.
...
- **Codex hook wrappers: collapse repeated adapter boilerplate** *(architecture / platform parity)* — Most Codex wrappers still inline repo discovery, `sys.path` setup, stdin JSON parsing, result printing, and `mcp__filesystem__read_file` -> `Read` mapping (example: `agents/codex/hooks/search-first.py:14-29`, `agents/codex/hooks/search-first.py:71-80`; same pattern appears in `auto-slice.py`, `read-guard.py`, `budget-observer.py`, `compact-trigger.py`, `truncate-output.py`). Shared helpers already exist in `agents/common/hooks/runtime.py:10-68` but are only partially adopted. Move wrappers toward a tiny `main(check_fn, maps, config_loader, native_output)` shape so the remaining Codex-specific code is only event translation, fallback config, and platform-specific message text. Acceptance: no wrapper defines its own `_resolve_repo`, direct `json.loads(sys.stdin.read())`, or local read-tool mapper unless it has a documented exception; smoke tests still cover nested cwd. **Pair with the nested-cwd relative-path bug fix (Bugs table)** — the same per-hook boilerplate is why that bug is universal across all 16 hooks rather than contained to one wrapper; this is the natural single place to fix the relative-path launcher once for all of them instead of patching `install.py`'s command generation ad hoc.
