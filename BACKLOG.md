...
- **Calibrate the token-count estimate** *(meta / measurement)* — `stats.py --calibrate` is already built (opt-in, network+key gated) but has never been run; `state/calibration.json` doesn't exist in either agent's state dir. Every "tokens saved" figure in README/CHANGELOG/DOCUMENTATION is currently an uncalibrated `chars÷4` estimate. Zero regression risk, zero dependencies — run once `ANTHROPIC_API_KEY` is available. Acceptance: `calibration.json` exists with a dated divisor; `stats.py`'s footer reads `chars÷N (calibrated <date>)` instead of `(uncalibrated)`.

- **Decide the compaction threshold height** *(measurement / decision)* — the test-isolation code fix (contaminating fixtures in `near_misses.jsonl`) shipped — see CHANGELOG.md; `near_misses.jsonl` is now clean going forward and covered by `conftest.py`'s CI regression gate, same as `events.jsonl`. What's left is the decision itself: `eb_telemetry_9jul26.md` §1 has 3,024 clean real samples from the `ever_better` repo's own dogfooding (43,374–1,414,558 chars, p50 411,074, p90 890,614), and 35.6% of those real sessions exceed this repo's configured 500,000-char threshold — over a third of ordinary sessions cross it. Open question for hofmockel: is that the intended aggressiveness, or should the threshold move? Separately, whether the return-code-2 nudge reliably leads to an actual `/compact` is still unanswered — this repo's own measured compactions are 1 all-time against many nudge-eligible samples; EB's separate budget-pressure pipeline shows a much higher real hit rate but isn't the same mechanism, so treat as directional only, not an answer.
