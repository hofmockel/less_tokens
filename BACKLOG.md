...
|---:|---|:---:|---|---|---|
| 1 | PT6 | P1 | Ready | Add hook-wiring assertions to `test_subagent_cap.py`/`test_subagent_fanout.py` | None (PT1 done) |
| 2 | PT7 | P1 | Ready | Make `codex_parity_audit.py` flag orphaned `.codex/hooks.json` wiring for scripts with no manifest Codex adapter | None |
...
- **PT7 — Close `codex_parity_audit.py`'s orphan-wiring blind spot** *(tool / enforcement parity)* — found during PT2's fix. For a `HookSpec` with no `codex=` wiring in the manifest (e.g. `truncate-output`, per CX28), `audit()` (`.claude/tools/codex_parity_audit.py:157-161`) appends a `missing-feature-parity | missing` row and `continue`s — it never checks whether `.codex/hooks.json` nonetheless still carries a stale entry for that script, and never adds such a case to `problems`. Confirmed empirically: before PT2's fix, `.codex/hooks.json` had a live orphaned `truncate-output.py` entry and `codex_parity_audit.py --root .` did not flag it anywhere in its output (contrary to `PT2`'s and `PT5`'s backlog text, which assumed it did). PT5 (gating this script in CI) will inherit this blind spot unless it's fixed first or noted as a known gap. Acceptance: for specs without a manifest `codex` adapter, `audit()` also checks `.codex/hooks.json` for any entry whose script matches and reports it as a problem; a regression test using a fixture `.codex/hooks.json` with such an orphan confirms it's caught.
